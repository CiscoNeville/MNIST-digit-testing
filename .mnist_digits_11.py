...
class EMNISTLettersDataset(Dataset):
    def __init__(self, images_file, labels_file, transform=None):
        ...
        # We want to include only E(5) and F(6) as NaN class
        included_labels = {5, 6}  # E and F only
        included_letters = "E,F"
        
        # Filter to keep only E and F
        valid_indices = []
        ...
        print(f"EMNIST letters: {len(self.images)} total, {len(valid_indices)} after filtering to include only {included_letters}")
...
if 'w' in train_classes:
    if os.path.exists(blank_images_path) and os.path.exists(blank_labels_path):
        ...
        datasets_for_training.append((whitespace_dataset, 0.1))
    else:
        missing_files = []
...
if 'errors' in visualize_options and misclassified_samples:
    ...
    for i, (true_label, pred_label, test_idx, original_idx) in enumerate(errors_to_show):
        ...
        if isinstance(test_dataset, Subset):
            parent_dataset = test_dataset.dataset
            img_to_show = parent_dataset.images[original_idx]
        else:
            img_to_show = test_dataset.images[original_idx]
